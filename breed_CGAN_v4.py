...
# Final edition of the generator and discriminator using DCGAN
...
def showExamples(training_data):
    ...
    for i in range(cols*rows):
        ...
        plt.imshow(np.transpose(img*0.7+0.6, (1,2,0)))
    plt.show()
...
class DiscriminatorModel(nn.Module):
    def __init__(self):
        ...
        self.hidden_1 = nn.Sequential(
            ...
            nn.Conv2d(ndf * 8, 1000, 4, 1, 0, bias=False),
            nn.BatchNorm2d(1000),
            nn.LeakyReLU(0.2, inplace=True)
        )
...
def train_GAN(dataloader, batch_size, G, D, loss, G_optimizer, D_optimizer, epochs, device):
    #holder for the result of each epoch
    G_loss = []
    D_loss = []
    counter = []
    latent_noise = torch.randn(16,100).to(device)
    latent_labels = torch.randint(0,120,(16, )).to(device)

    #call the train_loop and test_loop for each epoch
    for t in range(epochs):
        print(f"Epoch {t+1}\n-------------------------------")
        train_loop(dataloader, G, D, loss, G_optimizer, D_optimizer, G_loss, D_loss, counter, t, device, latent_noise, latent_labels)
    print("Done!")
...
def train_loop(dataloader, G, D, loss, G_optimizer, D_optimizer, G_loss, D_loss, counter, epochIdx, device, latent_noise, latent_labels):
    ...
    for batchIdx, (data, target) in enumerate(dataloader):
        ...
        if ((batchIdx + 1)% 150 == 0 and (epochIdx + 1)%2 == 0):
            figure=plt.figure(figsize=(8,6)) #8x6 inches window
            cols, rows = 4, 4
            with torch.no_grad():
                generated_data = G(latent_noise, latent_labels)
                for idx, x in enumerate(generated_data):
                    figure.add_subplot(rows, cols, idx+1)
                    plt.axis("off")
                    plt.title(latent_labels[idx].item())
                    plt.imshow(np.transpose(x.detach().cpu()*0.7+0.6, (1,2,0)))
            plt.show()
...
def main(argv):
    ...
    train_GAN(dataloader, batch_size, generator, discriminator, loss, generator_optimizer, discriminator_optimizer, epochs, device)
